...
- ASK before Phase 5 (`overdrive`) when SCOPE includes protected OTB surfaces (`src/lib/whatsapp.ts`, `src/content.config.ts`, `astro.config.mjs`, `src/content/products/otb.json`) because changes may affect CTA, schema, SEO, or build contract
...
- Anti-AI-slop/product gates: `Skill("otb-theme")` + `Skill("otb-usa")`
...
```
/design-improve landing                                  # full OTB landing surface
...
| 2 | bolder | `bolder.md` | frontend-specialist | OTB gold accent budget ≤10% surface in premium/restrained mode |
...
**PHASE_SPECIFIC_CONSTRAINT:** Respect OTB gold accent budget ≤10% surface (premium/restrained register). Only adjust intensity, weight, contrast, and structural boldness — never replace token anchors.
...
**PHASE_SPECIFIC_CONSTRAINT:** Maestro Template Test must still pass post-overdrive. Re-run the 6-gate self-check after the change. Pre-flight ASK gate fires if SCOPE includes protected OTB surfaces (`src/lib/whatsapp.ts`, `src/content.config.ts`, `astro.config.mjs`, `src/content/products/otb.json`).
...
| Skip `Skill("otb-theme")` in phase prompts | Load EVERY phase — impeccable LAYERS on top of otb-theme |
| Substitute impeccable palette for OTB anchors | impeccable enriches color strategy; OTB Navy/Gold stays canonical |
